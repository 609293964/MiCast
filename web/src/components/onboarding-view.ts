...
export function renderOnboarding(state: State): string {
  ...
  const afterXiaomi = step === "airplay2" || step === "complete";
  ...
  return `<main class="setup-page">
    ...
    <div class="setup-shell">
      <nav class="setup-steps" aria-label="部署进度">
        <div class="setup-step ${step === "access" ? "active" : "done"}"><span>${step === "access" ? "1" : "✓"}</span><div><strong>管理访问</strong><small>设置进入 MiCast 的方式</small></div></div>
        <div class="setup-step ${step === "xiaomi" ? "active" : afterXiaomi ? "done" : ""}"><span>${afterXiaomi ? "✓" : "2"}</span><div><strong>连接米家</strong><small>同步并控制你的音箱</small></div></div>
        ${supportsAirPlay2 ? `<div class="setup-step ${step === "airplay2" ? "active" : step === "complete" ? "done" : ""}"><span>${step === "complete" ? "✓" : "3"}</span><div><strong>AirPlay 2 <em class="feature-badge">实验性</em></strong><small>按需开启独立入口${step !== "complete" && !xiaomiReady ? ` <em class="step-tag">需先连接米家</em>` : ""}</small></div></div>` : ""}
      </nav>
        ...
        ${step === "access" ? `<div class="setup-copy"><h1>管理访问</h1><p>选择谁可以打开和管理这台 MiCast。</p></div>
          <form class="setup-form" data-setup-access>
            ...
            <div class="setup-credentials" data-setup-credentials>
              <label>用户名<input class="input" name="username" autocomplete="username" maxlength="64" value="admin" required></label>
          ...
          ${state.xiaomi.logged_in || state.qr.state === "confirmed"
            ? `<div class="setup-success"><span>✓</span><div><strong>米家已连接</strong><p>音箱将在进入应用后自动同步。</p></div></div>`
          ...
          <div class="setup-actions split"><button class="button plain" type="button" data-setup-skip>${state.xiaomi.logged_in ? "继续" : "暂时跳过"}</button>${state.qr.state === "expired" ? `<button class="button primary" type="button" data-setup-xiaomi>刷新二维码</button>` : ""}</div>`
          ...
          <form class="setup-form" data-setup-airplay2>
            ...
            <div class="setup-actions"><button class="button primary" type="submit">继续</button></div>
          </form>`
          : `<div class="setup-complete"><span>✓</span><h1>已经准备好了</h1><p>之后可以随时在设置中调整管理访问、米家和 AirPlay 2。</p><button class="button primary" type="button" data-setup-enter>开始使用</button></div>`}
      </section>
    </div>
  </main>`;
}
...
export function bindOnboarding(container: HTMLElement, handlers: {
  ...
  container.querySelector<HTMLElement>("[data-setup-skip]")?.addEventListener("click", handlers.onReview);
